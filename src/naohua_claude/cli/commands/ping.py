...
from naohua_claude.core.bus.commands import PongResult
from naohua_claude.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from naohua_claude.core.config import NaohuaConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: NaohuaConfig) -> None:
    try:
...
async def _ping(config: NaohuaConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{naohua_claude.__version__}"},
    }
