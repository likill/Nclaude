...
from naohua_claude.core.skills.loader import SkillLoader
from naohua_claude.core.transport.socket_client import IpcError, SocketClient
import pyfiglet

def make_banner(text: str = "NAOHUACLAUDE") -> str:
    raw = pyfiglet.figlet_format(text, font="standard")
    lines = [f"[bold #ff69b4]{line}[/bold #ff69b4]" for line in raw.splitlines() if line.strip()]
    lines.append("[dim]  输入消息开始对话  ·  键入 / 触发 skill  ·  Ctrl+C 退出[/dim]")
    return "\n".join(lines)
...
class NaohuaTuiApp(App[None]):
    """NaohuaClaude TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]NaohuaClaude[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]NaohuaClaude[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]NaohuaClaude[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 NaohuaTuiApp
def run(config: NaohuaConfig, replay_run_id: str | None = None) -> None:
    app = NaohuaTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
